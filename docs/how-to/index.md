# How-to guides

Goal-oriented guides for common tasks and integrations.

## Topics

- Configuration
  - [YAML config](configuration/yaml-config.md)
  - [Ignore rules](configuration/ignore-rules.md)
  - [Security values](configuration/security-values.md)
- Negative testing
  - [Overview](negative-testing/index.md)
- Generators
  - [Template generator](generators/template-generator.md)
  - [Custom templates](generators/custom-templates.md)
  - [Test-suite-writer](generators/test-suite-writer.md)
- Extension (custom rules/providers/generators/modules)
  - [Custom rules](extension/custom-rules.md)
  - [Custom providers](extension/custom-providers.md)
  - [Custom generators](extension/custom-generators.md)
  - [Custom modules](extension/custom-modules.md)
- Integration
  - [CI/CD](integration/ci-cd.md)
  - [RestAssured](integration/restassured.md)
  - [Spring Boot](integration/spring-boot.md)
- Troubleshooting
  - [Troubleshooting](troubleshooting.md)

