...
class InvalidTypeValidationRuleTest : ValidationRuleTest() {
    private val rule = InvalidTypeValidationRule()

    fun schemaProvider(): Stream<Arguments> = Stream.of(
        Arguments.of("Integer schema", IntegerSchema(), sequenceOf("abc")),
        Arguments.of("Number schema", NumberSchema(), sequenceOf("abc")),
        Arguments.of("Boolean schema", BooleanSchema(), sequenceOf("not_boolean")),
        Arguments.of("String schema", StringSchema(), emptySequence<Any>())
    )

    @ParameterizedTest
    @MethodSource("schemaProvider")
    @DisplayName("Invalid Type: apply() should return optional with invalid example when applicable")
    ...
    fun invalidTypeApplyTest(scenario: String, schema: Schema<*>, expected: Sequence<Any>) {
