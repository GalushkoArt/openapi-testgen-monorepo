...
detekt {
    // Reuse the shared ruleset from core to keep lint behavior consistent across feature modules.
    config.setFrom(files("$rootDir/../core/config/detekt.yml"))
    buildUponDefaultConfig = true
    baseline = file("$rootDir/../core/config/detekt-baseline.xml")
    parallel = true
    autoCorrect = false
}
...
kover {
    reports {
        ...
        total {
            verify {
                rule {
                    // Distribution module is an integration layer; core logic tested in core module
                    bound {
                        minValue = 70
                    }
                }
