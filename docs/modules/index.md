...
- [core](core.md)
- [example-value](example-value.md)
- [pattern-value](pattern-value.md)
- [pattern-support](pattern-support.md)
- [generator-template](generator-template.md)
...
| `example-value` | Schema-derived example generation and `SchemaValueProvider` SPI | See [example-value](example-value.md); used by `core`, extended by `pattern-value` |
...
| `pattern-support` | Optional pattern integration module for `core` | See [pattern-support](pattern-support.md); enabled by `distribution-bundle` defaults |
| `generator-template` | Mustache-based source-code generation (`template`) | See [generator-template](generator-template.md); enabled by `distribution-bundle` defaults |
...
- Owns `SchemaValueProvider`, `SchemaExampleValueGenerator`, response example extraction (`ResponseExampleExtractor` with a pluggable `ResponseBodyGenerator`), and schema merging support
- Java-friendly standalone API: options presets with `with*` methods, `@JvmOverloads` entry points, SAM-convertible interfaces
- Depends on `model`
- Start with [Example-value module](example-value.md) for public API and standalone usage; see also [SPI](../reference/spi.md#value-providers) and [Distribution settings](../reference/distribution-settings.md#testgenerationsettingsexamplevalues)
...
- Start with [Pattern-support module](pattern-support.md) for wiring and embedding; see also [Distribution settings](../reference/distribution-settings.md#testgenerationsettingspatterngeneration) and [Rules catalog](../reference/catalogs/rules-catalog.md)

## `generator-template`

- Contributes the `template` generator module
- Owns built-in RestAssured template sets and custom-template loading
- Start with [Generator-template module](generator-template.md) for module wiring and template resolution; see [Generators](../how-to/generators.md#template-generator) for the options table
